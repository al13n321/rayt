...
namespace rayt {
	...
	namespace import_obj_scene_old {
    ...
    void ImportObjSceneOld(int max_level, int nodes_in_block, std::string obj_file_name, std::string out_tree_file_name, bool print_report) {
		vector<import_obj_scene_old::Triangle> tris;
        import_obj_scene_old::ReadObjFile(obj_file_name, tris);
        ...
        pair<dvec3, dvec3> bound = import_obj_scene_old::BoundingCube(tris);
        import_obj_scene_old::NodeData d = import_obj_scene_old::BuildOctree(tris.begin(), static_cast<int>(tris.size()), bound.first, bound.second, 0, max_level, writer);
