	...
	namespace stored_octree_builder_impl {
		...
		public:
			...
			int CommitBlock(const vector<LinkedUnfinishedBlock> &parts, int parent_block_index) {
				...
				block.header.roots_count = static_cast<int>(parts.size());
				block.header.roots[0].parent_pointer_index = parts[0].parent_pointer_index;
				block.header.roots[0].parent_pointer_value = (block.header.block_index << 9) + (1 << 8) + parts[0].parent_pointer_children_mask;
				...
				for (int i = 1; i < static_cast<int>(parts.size()); ++i) {
					...
					block.header.roots[i].parent_pointer_value = (block.header.block_index << 9) + (1 << 8) + parts[i].parent_pointer_children_mask;
			...
			UnfinishedBlock* BuildRecursive(vector<Tnode*> &root_children, int reserved_nodes_in_block) {
				...
				res->used_nodes_count = static_cast<int>(root_children.size());
